...
let $ills := 
  (
  gen:generate(doc("/templates/cold.xml")/template, 5, 2, 99, 45), 
  gen:generate(doc("/templates/flu.xml")/template, 2, 2, 12, 50),
  gen:generate(doc("/templates/flu.xml")/template, 4, 13, 55, 25),
  gen:generate(doc("/templates/flu.xml")/template, 2, 56, 80, 35),
  gen:generate(doc("/templates/allergies.xml")/template, 2, 4, 99, 15)
  )
return 
( text{ "tot ills", count($ills), "tot symptoms", count($ills//symptom)}
,
$ills[1]
)